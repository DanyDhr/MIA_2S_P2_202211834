	...
	*comando = (*comando)[1:]
	...
	for _, valor := range *comando {
		...
		if bandera == "-path" {
			...
			// Si el valor tiene comillas, se procesa
			if strings.Contains(banderaValor, "\"") {
				valorPath = strings.Replace(banderaValor, "\"", "", -1)
				*comando = (*comando)[1:]
			}
			*comando = (*comando)[1:] // Eliminar el parámetro de la lista
		} else {
	...
	if err := os.Remove(valorPath); err != nil {
		fmt.Printf("Error al eliminar el disco: %s\n", err)
		respuesta += fmt.Sprintf("Error al eliminar el disco: %s\n", err)
	} else {
		fmt.Printf("Disco eliminado: %s\n", valorPath)
		respuesta += fmt.Sprintf("Disco eliminado: %s\n", valorPath)
	}
